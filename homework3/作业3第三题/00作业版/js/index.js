...
function renderNews() {
  ...
  // 获取了一个名为 "tpl" 的HTML模板元素的内容，并将其存储在 tpl 变量中，以便后续用于渲染新闻内容。
  //【需要完成代码1】：此处实现新闻模板内容的渲染
  let RenderHtml = newsDataRender
    .map((item) => {
      // 回顾：数组map用法
      // 替换script模板内容
      let result = tpl
...
async function run() {
  ...
  skipInput.addEventListener("keydown", function (event) {
    if (event.keyCode === 13) {
      let pageNum = parseInt(skipInput.value);
      if (pageNum >= 1 && pageNum <= asAll.length) {
        p = pageNum;
        changePageClass(p);
        renderNews();
      }
